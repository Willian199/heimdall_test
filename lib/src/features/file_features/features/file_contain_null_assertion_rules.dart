...
extension FileContainNullAssertionPredicateRules on FilePredicateBuilder {
  ...
  FilePredicateBuilder containNullAssertion() {
    const match = nullAssertionMatch;
    return satisfy(HeimdallPredicate('contain a null assertion', (item, project) => hasSyntaxMatch(item, project, match)));
  }
  ...
  FilePredicateBuilder notContainNullAssertion() {
    const match = nullAssertionMatch;
    return satisfy(HeimdallPredicate('not contain a null assertion', (item, project) => !hasSyntaxMatch(item, project, match)));
  }
}
