...
void main() {
  ...
  test('an inherited static field does not shadow the imported class', () {
    expect(project.parseErrors, isEmpty);

    final report = Heimdall.methods().that().haveName('callStaticInherited').should().callStaticMethod('Tools', 'run').check(project);
